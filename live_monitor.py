...
import pynvml
import msvcrt  # Built-in Windows library for non-blocking keystroke detection
import ctypes  # <--- ADD THIS IMPORT FOR WINDOWS API CALLS


# ─── WINDOWS TERMINAL FORCE-MAXIMIZE RE-LAUNCHER ───
# Check an environment variable to prevent an infinite loop of relaunching
if os.name == 'nt' and "ALPHAMINER_MAXIMIZED" not in os.environ:
    os.environ["ALPHAMINER_MAXIMIZED"] = "1"
    # Command Windows Terminal (wt) to open a new maximized (-M) tab running this exact script
    launch_cmd = f'wt -M new-tab python "{abs_path}"' if (abs_path := os.path.abspath(__file__)) else ""
    
    # Execute the command and gracefully close the original, unmaximized tab
    if os.system(launch_cmd) == 0:
        sys.exit(0)
...
#This should match your Docker container. You probably don't need to change this
...
# 📊 WHATTO MINE CUSTOMIZATION CONFIGURATIONS
# WhatToMine automatically deducts a 15% discount off estimated rewards by default.
# Leave at 15.0 to change nothing. Change to 0.0 to automatically restore baseline full rewards.
...
# View State Variable
current_view = "DASHBOARD"  # Can toggle between "DASHBOARD" and "ERROR_VIEW"
...
def load_history_from_local_file():
    ...
    if not os.path.exists(HISTORY_FILE_NAME):
        print(f"ℹ️ No local {HISTORY_FILE_NAME} found yet. Skipping parser.", flush=True)
        return

    try:
        file_size_bytes = os.path.getsize(HISTORY_FILE_NAME)
        if file_size_bytes > 50 * 1024 * 1024:  # 50 MB warning limit
            size_mb = file_size_bytes / (1024 * 1024)
            print(f"\n⚠️ WARNING: '{HISTORY_FILE_NAME}' is quite large ({size_mb:.2f} MB). Re-indexing may take a moment...", flush=True)
    except Exception:
        pass

    print(f"Parsing local '{HISTORY_FILE_NAME}' to rebuild history...", flush=True)
    ...
    try:
        ...
        print(f"⚠️ Error reading log file: {e}", flush=True)
        return
    ...
    # Process up to the last 10,000 file entries max to prevent lockups
    ...
    print(f"🔄 Successfully recovered {len(dashboard_history)} historical records from local log file!", flush=True)

# Initialize NVML
print("🔧 Initializing NVML API drivers...", flush=True)
...
try:
    ...
    nvml_enabled = False
print(f"   NVML Status: {'🟢 ONLINE (' + gpu_name_str + ')' if nvml_enabled else '🟡 OFFLINE (Using fallback logic)'}", flush=True)
...
def fetch_market_data():
    ...
    print("\n🌐 Querying dynamic financial indicators from APIs...", flush=True)
    ...
    try:
        ...
        print("   • WhatToMine (Rewards Matrix)  : 🟢 OK", flush=True)
    except Exception:
        print("   • WhatToMine (Rewards Matrix)  : ❌ FAIL", flush=True)
        pass 
    ...
    try:
        ...
        btc_ok = True
        print("   • CoinDesk (Live BTC Index)   : 🟢 OK", flush=True)
    except Exception:
        try:
            ...
            print("   • CoinDesk / Backup BTC APIs   : ❌ FAIL", flush=True)
            pass
    ...
    try:
        ...
        print("   • AlphaPool (Global Metrics)   : 🟢 OK", flush=True)
    except Exception:
        print("   • AlphaPool (Global Metrics)   : ❌ FAIL", flush=True)
        pass 
    ...
    if WALLET_ADDRESS:
        try:
            ...
            print("   • AlphaPool (Wallet Ledger)    : 🟢 OK", flush=True)
        except Exception:
            print("   • AlphaPool (Wallet Ledger)   : ❌ FAIL", flush=True)
            pass
...
# Persistent variables for immediate redrawing engines
current_hr = 0.0
avg_hr = 0.0
avg_pool_equiv = 0.0
current_w = 0.0
...
print(f"\nWindows Cumulative Monitor for '{container_name}'")
load_history_from_local_file()

print("\n🔄 Phase 1: Contacting Docker Engine and reading recent active container logs...", flush=True)
...
try:
    ...
    if history_result.returncode != 0:
        print(f"❌ Failed to fetch logs. Is container '{container_name}' running? Docker Error:\n{history_result.stdout}", flush=True)
        sys.exit(1)
        
    historical_lines = history_result.stdout.splitlines()
    print(f"   Loaded {len(historical_lines)} lines of container history. Parsing patterns...", flush=True)
    ...
    # Finalize state variables from history load
    ...
    print("\n" + "="*75, flush=True)
    print(f"📊 LOGS SUMMARY (Container start: {start_time.strftime('%Y-%m-%d %H:%M:%S')})", flush=True)
    print(f"  • Container Runtime: {total_elapsed_mins:.1f} minutes", flush=True)
    print(f"  • Cumulative Shares: {total_shares} ({shares_per_min:.2f} shares/min)", flush=True)
    print(f"  • Avg Active Hashrate: {avg_hr:.2f} TH/s", flush=True)
    print(f"  • Total Memory Logs Tracked: {len(dashboard_history)}", flush=True)
    print("="*75, flush=True)
    print("\n🔄 Phase 2: Launching main execution matrix loops...\n", flush=True)

except Exception as e:
    print(f"❌ Error during historical parsing: {e}", flush=True)
    sys.exit(1)
...
try:
    while True:
        force_render = False
        any_new_status = False

        if msvcrt.kbhit():
            key_pressed = msvcrt.getch()
            try:
                char_pressed = key_pressed.decode('utf-8').lower()
            except Exception:
                char_pressed = ""
                
            if char_pressed == "`":
                current_view = "ERROR_VIEW" if current_view == "DASHBOARD" else "DASHBOARD"
                os.system('cls' if os.name == 'nt' else 'clear')
                force_render = True  # Triggers instantaneous redraw engine
                while msvcrt.kbhit(): msvcrt.getch()

            elif char_pressed == "a" and current_view == "ERROR_VIEW":
                total_errors = 0
        ...
        # Executes instantly upon user key toggle OR when fresh data gets parsed.
        if any_new_status or force_render:
            ...
            if api_data:
                ...
                if wtm_daily_revenue_usd > 0: 
                    # Reverses the 15% discount fraction seamlessly based on configuration bounds
            ...
            if current_view == "DASHBOARD":
                ...
                print(f"  🔌 Power & Thermal: {current_w:.1f}W @ {temp_c}°C (Session Avg: {session_avg_w:.1f}W @ {session_avg_temp:.1f}°C) | Device: {gpu_name_str}", flush=True)
                ...
                if last_api_fetch_time:
                    ...
                    print(f"  🌐 Financial API Data : Next update in {time_remaining}s", flush=True)
                else:
                    print(f"  🌐 Financial API Data : Next update imminent", flush=True)
                
                print(f"  🚨 Health Status  : {error_status}", flush=True)                
                if total_errors > 0:
                    print(f"      ↳ [ Press ` key to view/clear all {total_errors} untruncated logs in full screen ]", flush=True)
            ...
            elif current_view == "ERROR_VIEW":
                print("\n" + "═" * 85, flush=True)
                print(f"  🚨 [CRITICAL HEALTH EXAMINER]  • Total Logged Incidents: {len(recent_errors_log)}", flush=True)
                print("═" * 85, flush=True)
                print("  [⌨️  CONTROLS]: Press [ A ] to Acknowledge & Reset Counters  |  Press [ ` ] to Close View", flush=True)
                print("-" * 85 + "\n", flush=True)
                if not recent_errors_log:
                    print("    🟢 Log backlog is clear. No active alerts recorded.", flush=True)
                else:
                    for entry in recent_errors_log:
